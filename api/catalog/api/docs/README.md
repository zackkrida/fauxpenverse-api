This documentation is focused towards consumers who are using the fauxpenverse API.
The developer documentation for the fauxpenverse API can be found [here]().

# Introduction

The fauxpenverse API is a system that allows programmatic access
...
per day and 100 requests per minute. Additionally, fauxpenverse can give your key
...
Before using the fauxpenverse API, you need to register access via OAuth2. This can
...
In order to use the fauxpenverse API endpoints, you need to include access token in
...
$ curl \
  ...
  "https://api.fauxpenverse.engineering/v1/auth_tokens/token/"
```
...
$ curl \
  ...
  "https://api.fauxpenverse.engineering/v1/images?q=test"
```
...
| client ID         | a publicly exposed string used by fauxpenverse API to identify the application                                                                                                                |
| client secret     | a private string that authenticates the identity of the application to the fauxpenverse API                                                                                                   |
...
[contributing on Github](https://github.com/zackkrida/fauxpenverse-api), here’s a
todo list to get started.

- Read up about [Django REST Framework](https://www.django-rest-framework.org/),
  which is the framework used to build fauxpenverse API
...
- Run the server locally by following this
  [link](https://github.com/zackkrida/fauxpenverse-api#running-the-server-locally)
- Update documentation or codebase
- Make sure the updates passed the automated tests in this
  [file](https://github.com/zackkrida/fauxpenverse-api/blob/master/.github/workflows/integration-tests.yml)
- Commit and push
- Create pull request
